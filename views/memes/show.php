...
<div class="row">
    <div class="col-md-8">
        <div class="card-body">
            <img class="card-img-top" src="<?=MEMES_ROOT_PATH.$viewmodel['meme']['path']?>" alt="Card image cap" style="max-height: 80vh;width: auto; max-width: 100%;">
        </div>
    </div>
        <div class="col-md-4 pt-4">
            <div class=" p-2 d-flex justify-content-between align-items-start border-bottom">
                <a href="<?=ROOT_PATH?>users/show/<?=$viewmodel['meme']['author_name']?>">
                    <img src="<?=AVATARS_PATH;?><?=$viewmodel['meme']['avatar']?>" alt="avatar" class="rounded-circle w-auto" style="height: 100px; !important">
                </a>    
                <div class="d-flex flex-column justify-content-between">
                    <p><a href="<?=ROOT_PATH?>users/show/<?=$viewmodel['meme']['author_name']?>" style="color: black;"><?=$viewmodel['meme']['author_name']?></a></p>
                    <p><strong>Points: <?=$viewmodel['meme']['points']?></strong></p>
                </div>
            </div>
            <p class="p-3 border-bottom">
            Lorem ipsum dolor sit amet consectetur adipisicing elit. Ipsum ad illo eaque accusantium rerum harum consequatur qui ducimus aperiam, sit dignissimos quam, nesciunt necessitatibus temporibus ea voluptates amet minus eius.
            </p>
            ...
            <div>
                <form method="POST" action="<?=ROOT_PATH?>memes/show/<?=$viewmodel['meme']['id']?>">
                    <div class="form-group">
                        ...
                        <textarea class="form-control" id="comment" rows="5" name="body" placeholder="Musisz być zweryfikowanym użytkownikiem aby dodać komentarz!" <?=(isset($_SESSION['user_data']['is_verified']) && $_SESSION['user_data']['is_verified'] == true)?'':'disabled';?>></textarea>
        ...
        <?php foreach($viewmodel['comments'] as $comment): ?>
            <li class="media border card-footer py-2 px-3 w-100 my-3">
                <a href="<?=ROOT_PATH?>users/show/<?=$comment['name']?>">
                    <img src="<?=AVATARS_PATH;?><?=$comment['avatar']?>" class="mr-3" alt="avatar" style="height:64px;width:auto;">
                </a>
